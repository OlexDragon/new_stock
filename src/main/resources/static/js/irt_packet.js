...
// Packet Group ID
const packetGroupId = {}
packetGroupId.measurement = 4;	/* Measurement: device status, content is product specific. */
packetGroupId.deviceInfo  = 8;	/* Device information: generic command. */

class Header{
	constructor(type, groupId){

		this.type 		= (type == undefined ? packetType.request : type);				// byte	type;		0
		this.packetId 	= Math.floor(Math.random() * 32767 );							// short packetId;	1,2 - random value
		this.groupId 	= (groupId == undefined ? packetGroupId.deviceInfo : groupId);	// byte groupId;	3
		this.reserved	= 0;															// short reserved;	4,5
		this.error		= 0;															// byte errorCode;	6
	}
	toBytes(){
...
class Parameter{
	...
	toBytes(){
		...
		return [this.code, sizeBytes[0], sizeBytes[1]];
	}
}
